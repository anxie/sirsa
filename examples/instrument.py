...
def add_command_line_args_to_variant_spec(variant_spec, command_line_args):
    ...
    variant_spec['environment_params']['training']['kwargs'].update({
        'observe_context': command_line_args.observe_context,
        'task_id': command_line_args.task_id,
        'sample_id': command_line_args.sample_id,
        'num_tasks': command_line_args.num_tasks,
        'env_type': command_line_args.env_type,
        'history_length': command_line_args.history_length,
    })

    variant_spec['algorithm_params']['kwargs'].update({
        'threshold_iterations': command_line_args.threshold_iterations,
        'cvar_alpha': command_line_args.cvar_alpha,
        'cvar_samples': command_line_args.cvar_samples,

        'latent_dim': command_line_args.latent_dim,
        'state_dim': command_line_args.state_dim,
        'pretrain_iterations': command_line_args.pretrain_iterations,
        'predict_context': command_line_args.predict_context,
        'ensemble_size': command_line_args.ensemble_size,
        'history_length': command_line_args.history_length,

        'redq_subset_size': command_line_args.redq_subset_size,
        'num_Qs': command_line_args.num_qs,
    })

    variant_spec['Q_params']['kwargs'].update({
        'num_Qs': command_line_args.num_qs,
    })

    variant_spec['replay_pool_params']['kwargs'].update({
        'num_tasks': command_line_args.num_tasks,
        'percentile': command_line_args.percentile,
        'threshold_samples': command_line_args.threshold_iterations,
    })

    variant_spec['sampler_params']['kwargs'].update({
        'update_belief': command_line_args.update_belief,
        'latent_dim': command_line_args.latent_dim,
    })

    if command_line_args.oracle:
        variant_spec['Q_params']['kwargs'].update({
            'observation_keys': ('observation', 'context'),
        })
        variant_spec['policy_params']['kwargs'].update({
            'observation_keys': ('observation', 'context'),
        })
        variant_spec['exploration_policy_params']['kwargs'].update({
            'observation_keys': ('observation', 'context'),
        })
    elif command_line_args.observe_context:
        variant_spec['Q_params']['kwargs'].update({
            'observation_keys': ('observation', 'context'),
        })
        variant_spec['policy_params']['kwargs'].update({
            'observation_keys': ('observation', 'center', 'radius'),
        })
        variant_spec['exploration_policy_params']['kwargs'].update({
            'observation_keys': ('observation', 'center', 'radius'),
        })
...
def generate_experiment_kwargs(variant_spec, command_line_args):
    ...
    if variant_spec['restore']:
        experiment_dir = variant_spec['restore']
    else:
        experiment_dir = os.path.join(local_dir, experiment_id)
    if os.path.exists(experiment_dir):
        for d in os.listdir(experiment_dir):
            if command_line_args.trial_name_template in d:
                trial_dir = os.path.join(experiment_dir, d)
                checkpoints = [e for e in os.listdir(trial_dir) if 'checkpoint_' in e]
                if checkpoints:
                    checkpoint = natsorted(checkpoints)[-1]
                    variant_spec['restore'] = os.path.join(trial_dir, checkpoint, '')
    ...
    experiment_kwargs = {
        ...
        'restore': variant_spec['restore'],  # Defaults to None
    }
