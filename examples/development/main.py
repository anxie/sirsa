...
class ExperimentRunner(tune.Trainable):
    ...
    def _replay_pool_pickle_path(self, checkpoint_dir):
        if isinstance(self.replay_pool, MultitaskPercentileReplayPool):
            return os.path.join(checkpoint_dir, 'replay_pool_{task_id}.pkl')
        else:
    ...
    def picklables(self):
        ...
        if self.algorithm._encode_history:
            p.update({
                'encoder_weights': [encoder.get_weights() for encoder in self.algorithm._encoders],
            })
        return p
    ...
    def _restore(self, checkpoint_dir):
        ...
        # restore weights of encoder
        if self.algorithm._encode_history:
            for i, encoder in enumerate(self.algorithm._encoders):
                encoder.set_weights(picklable['encoder_weights'][i])
